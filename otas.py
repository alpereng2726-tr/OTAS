...
class OTAS:
    ...
    #uyarı labeli
        ...
        uyarılabel=ctk.CTkLabel(
        card,
        text="⚠ Bu sistem hastaların doğru yönlendirilmesi amacıyla geliştirilmiştir.\n"
            "Kasıtlı olarak yanlış bilgi verilmesi gerçek acil hastaların tedavisini geciktirebilir.\n" \
            "SİSTEMİ KANDIRAN İNSANLAR ENGELLENECEKTİR",
        font=("Arial", 19),
        text_color="#C62828",
        justify="center",
        wraplength=600
   ...
   # ---------------- SAYFA 2 ----------------
    def sayfa2(self, isim, soyisim):
        ...
        gif_cercevesi = ctk.CTkFrame(self.card, fg_color="transparent")
        ...
        self.oto_or_manuel = ctk.CTkButton(
            self.root,
            text="MANUEL",
            width=self.rw(0.1),
            height=self.rh(0.05),
            ...
            fg_color=self.PRIMARY,
            hover_color="#070900",
            command=lambda: (self.changemod())
        )
        self.oto_or_manuel.place(x=30, y=100)
        ...
        # 2. DEVAM ET BUTONU (Başlangıçta disabled)
     ...
     #manuel ve otomatik mod içinde frame lazım hocam 
    # ----------------- MANUEL MOD FRAME TASARIMI -----------------
        
        self.manual_frame = ctk.CTkFrame(
            self.root, 
            corner_radius=30,
            width=self.rw(0.6),    
            height=self.rh(0.85),   # Butonun rahat sığması için yüksekliği biraz artırdık
            fg_color="#dbdbdb",
            bg_color="#fdeded"
        )
        # İçindeki elemanların frame boyutunu ezmesini engellemek için propagate'i kapatıyoruz
        self.manual_frame.pack_propagate(False) 
        
        manuel_title = ctk.CTkLabel(
            self.manual_frame,
            text="MANUEL VERİ GİRİŞİ",
            font=("Arial", max(22, int(self.sw * 0.016)), "bold"),
            text_color="#333333"
        )
        manuel_title.pack(pady=(20, 10))

        label_font = ("Arial", max(14, int(self.sw * 0.011)), "bold")
        entry_font = ("Arial", max(16, int(self.sw * 0.012)))
        entry_w = 420
        entry_h = 45 # Yüksekliği 55'ten 45'e çekerek butona yer açtık

        # --- Nabız ---
        ctk.CTkLabel(self.manual_frame, text="Nabız (Atım/Dakika)", font=label_font, text_color="#555555").pack(pady=(5, 2))
        self.nabiz_entry = ctk.CTkEntry(self.manual_frame, width=entry_w, height=entry_h, font=entry_font, justify="center")
        self.nabiz_entry.pack(pady=2)

        # --- Büyük Tansiyon ---
        ctk.CTkLabel(self.manual_frame, text="Büyük Tansiyon (SYS)", font=label_font, text_color="#555555").pack(pady=(5, 2))
        self.buyuk_entry = ctk.CTkEntry(self.manual_frame, width=entry_w, height=entry_h, font=entry_font, justify="center")
        self.buyuk_entry.pack(pady=2)

        # --- Küçük Tansiyon ---
        ctk.CTkLabel(self.manual_frame, text="Küçük Tansiyon (DIA)", font=label_font, text_color="#555555").pack(pady=(5, 2))
        self.kucuk_entry = ctk.CTkEntry(self.manual_frame, width=entry_w, height=entry_h, font=entry_font, justify="center")
        self.kucuk_entry.pack(pady=2)

        # --- SpO2 ---
        ctk.CTkLabel(self.manual_frame, text="Oksijen Satürasyonu (SpO₂ - %)", font=label_font, text_color="#555555").pack(pady=(5, 2))
        self.oksijen_entry = ctk.CTkEntry(self.manual_frame, width=entry_w, height=entry_h, font=entry_font, justify="center")
        self.oksijen_entry.pack(pady=2)

        # --- DEVAM ET BUTONU ---
        # command kısmındaki parantez () kaldırıldı!
        self.btn_manuel_devam = ctk.CTkButton(
            self.manual_frame,
            text="DEVAM ET",
            width=entry_w,
            height=55,
            ...
            fg_color=self.PRIMARY,
            hover_color="#00796B",
            command=self.manuel_kaydet_ve_gec
        )
        self.btn_manuel_devam.pack(pady=(25, 15))

    def manuel_kaydet_ve_gec(self):
            global nabız
            global satrasyon
            global ktansiyon
            global btansiyon
            nabız=self.nabiz_entry.get()
            satrasyon=self.oksijen_entry.get()
            ktansiyon=self.kucuk_entry.get()
            btansiyon=self.buyuk_entry.get()
            self.activateDevam()
    ...
    def changemod(self):
        if self.changemodvar==0:
            print(self.changemodvar)
            self.changemodvar=1
            self.oto_or_manuel.configure(text="OTOMATİK")

            self.card.place_forget()
            self.manual_frame.place(relx=0.5, rely=0.5, anchor="center")
        else:
            print(self.changemodvar)
            self.changemodvar=0
            self.oto_or_manuel.configure(text="MANUEL")
    ...
    def activateDevam(self):
        if self.changemodvar==0: #mod otomatikte ise
        ...
        elif self.changemodvar==1: #mod manuel ise
            if int(nabız)>=120 or int(nabız)<=50:
                self.acil_uyari()
            elif int(satrasyon)<90:
                self.acil_uyari()
            elif int(btansiyon)<=90 or int(btansiyon)>=180:
                self.acil_uyari()
            elif int(ktansiyon)>=90 or int(ktansiyon)<=50:
                self.acil_uyari()
            else:
                self.normaldegerler()#hocam buraya değerleriniz normaldir sorulara geçin die bir label lazım
    ...
    def olcum_bitti(self):
            ...
            onay_yazisi = ctk.CTkLabel(self.card, text="Ölçüm tamamlandı!", font=("Arial", 24), text_color="green")
    ...
    def normaldegerler(self):
        self.temizle()
        color = self.PRIMARY
        text = "değerleriniz normaldir.\nsorulara geçebilirsiniz."
        ...
        self.root.after(3000,self.sorular)
